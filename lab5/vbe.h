 ...
 */
 ...
 typedef uint32_t real_mode_far_pointer;    // Defining  'real mode far pointer'
...
typedef struct {
    ...
    real_mode_far_pointer VideoModePtr;       /* Pointer to VideoModeList */
    uint16_t TotalMemory;                     /* Number of 64kb memory blocks */

    /* Added for VBE 2.0 */
    uint16_t OemSoftwareRev;                  /* VBE implementation Software revision */
    real_mode_far_pointer OemVendorNamePtr;   /* Pointer to Vendor Name String */
    real_mode_far_pointer OemProductNamePtr;  /* Pointer to Product Name String */
    real_mode_far_pointer OemProductRevPtr;   /* Pointer to Product Revision String */
 ...
 *
 * Initializes unpacked vbe_ctrl_info_t structure passed as an address,
 *  by calling VBE function 0x01
 *  Return VBE Controller Information and unpacking the ControllerInfoBlock struct
 *  returned by that function.
 *
 * @param vci_p address of vbe_ctrl_info_t structure to be initialized
 *
 * @return 0 on success, non-zero otherwise
 */
...
/**
 * @briefs Prints the information of the VBE controller
 *
 * @param adress of the vbe_ctrl_info_t structure
 */
void vbe_print_ctrl_info(vbe_ctrl_info_t *vci_p);
 ...
 // Symbolic Constants
 #define VBE_LMINIT_FAILED             1
 #define VBE_LMALLOC_FAILED            2
 #define VBE_SYSINT86_FAILED           3
 #define GET_MODE_INFO_FAILED          4
 #define GET_CTRL_INFO_FAILED          5
 #define VIDEO_MODES_TERMINATION_WORD  0xFFFF

 #define VBE_MAJOR_VERSION(x)    ( ((x) & 0xFF00) >> 8 )
 #define VBE_MINOR_VERSION(x)    (  (x) & 0x00FF )

 #define VBE_MODE_INFO_BLOCK_SIZE    sizeof(vbe_mode_info_t)
 #define VBE_CTRL_INFO_BLOCK_SIZE    sizeof(vbe_ctrl_info_t)
