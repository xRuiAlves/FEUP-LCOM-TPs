...
void drawBitmapWithTransparency(Bitmap* bmp, int x, int y, char* buffer) {
    ...
    unsigned short* image   = (unsigned short*) bmp->bitmapData;
    unsigned short* destiny = (unsigned short*) buffer;

    int i , j;

    for (i=height-1 ; i>=0 ; i--){
        for (j=0 ; j<width ; j++){
            if (*image != TRANSPARENCY_COLOR)
                *(destiny + (y+i)*getHorResolution() + (x+j)) = *(image);
            image ++;
        }
    }
}
